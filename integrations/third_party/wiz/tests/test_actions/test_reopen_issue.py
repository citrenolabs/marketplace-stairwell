...
JSON_RESULT_OBJECT: datamodels.Issue = data_parser.build_update_issue_object(common.REOPEN_ISSUE)

SUCCESS_OUTPUT_MESSAGE: str = (
    f"Successfully reopened issue with ID {ISSUE_ID} in {constants.INTEGRATION_NAME}."
)
FAILED_OUTPUT_MESSAGE: str = (
    f'Error executing action "{constants.REOPEN_ISSUE_SCRIPT_NAME}"\nReason: '
    f"Issue with ID {common.INVALID_ISSUE_ID} wasn't found in "
    f"{constants.INTEGRATION_NAME}."
)

DEFAULT_PARAMETERS: Mapping[str, str] = {"Issue ID": ISSUE_ID}
...
def test_reopen_issue_action_success(
    wiz: Wiz,
...
def test_reopen_issue_action_failure(
    wiz: Wiz,
    script_session: WizSession,
    action_output: MockActionOutput,
) -> None:
